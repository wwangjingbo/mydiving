...
class edge_aggregation(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_size=(1024, 512), activation='relu', discrim=False, dropout=-1, edge_types=1):
...
class MS_HGNN_hyper(nn.Module):
    """Pooling module as proposed in our paper"""
    def __init__(
        self, embedding_dim=512, h_dim=512, mlp_dim=1024, bottleneck_dim=1024,
        activation='relu', batch_norm=True, dropout=0.0, nmp_layers=4, scale=2, vis=False, actor_number=128
    ):
    ...
    def forward(self, h_states, corr, mask, viz=False):
        ...
        if self.nmp_layers <= 1:
            ...
            for nmp_l, nmp_mlp in enumerate(self.nmp_mlps):
                if nmp_l%2==0:
                    node_feat = nmp_mlp(self.edge2node(edge_feat,node_feat,H,node2edge_idx)[0]) 
                    node2edge_idx += 1
                else:    
                    edge_feat, _ = nmp_mlp(self.node2edge(node_feat, H, idx=node2edge_idx)[0]) 
...
class HypergraphNet(nn.Module):
    def __init__(self, hidden_dim=256, num_classes=11, output_dim=1,num_frames=128):
    
        super(HypergraphNet, self).__init__()
        self.hyper_model = MS_HGNN_hyper()
        ...
        self.transformer = transformer(smpl=None, frame_length=num_frames)
        self.temconv11 = nn.Conv1d(128, 1, 1)
        self.temconv21 = nn.Conv1d(128, 1, 1)
        
    def forward_v1(self, images):
        
        B, T, C, H, W = images.shape
        images_reshaped = images.view(B * T, C, H, W)
        images = self.resnet_model(images_reshaped)
        out = F.adaptive_avg_pool2d(images, (1, 1))
        out = out.view(B*T, -1)
        out = out.unsqueeze(0)

        valid_mask = torch.ones(B, T, device=out.device)
        out = out * valid_mask[:, :, None]
        hyper_mask = torch.matmul(valid_mask[:, :, None], valid_mask[:, None, :])
        images_norm = F.normalize(out, p=2, dim=-1)

        hyper_corr = torch.matmul(images_norm, images_norm.transpose(1, 2))
        x, _ = self.hyper_model(out, hyper_corr, hyper_mask)

        class_out = self.classifier(x)
        score_out = self.regressor(x)
    ...
    def forward_v2(self, images):
        ...
        transformer_out = self.transformer.forward_features(x)
