...
from dataset_T import get_dataloaders
...
def train(model, train_loader, loss_fn, optimizer, epoch, device, scaler, writer, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            #         images_reshaped, nrow=10, normalize=True, scale_each=True)
            #     writer.add_image("Augmented image", img_grid, i)

            outputs = model.forward_v2(images)
        ...
        reg_outputs = torch.abs(reg_outputs)
    ...
    all_pred_scores = np.concatenate([np.atleast_1d(score.cpu().numpy()) for score in all_pred_scores])
...
def main():
    ...
    parser.add_argument('--arch', default="ResNet50_T", type=str)
